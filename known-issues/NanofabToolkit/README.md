...
| [`PreciousMetalReader.md`](PreciousMetalReader.md) | CORES precious-metal billing extractor | CORES creds: env-var preference added (2026-06-22); shared CORES token **rotated 2026-06-29** → local `auth.py` now 403, so the env rollout is **now required** to restore the tool (`auth.py` never committed) |
