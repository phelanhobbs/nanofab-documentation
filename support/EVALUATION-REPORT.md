...
- **#8 filetransfer remote path** (found during the full source validation) — ✅ Fixed. The doc now documents both forms: the deployed per-machine scripts' `/Users/phelanh/Desktop/Logs/<MACHINE>` (the operative path) and the template's divergent Windows-style `C:\Users\phelanh\Desktop\Logs\MACHINE` placeholder (`FileTransferTemplate.ps1:32`), with a warning to copy a per-machine script rather than the template.
