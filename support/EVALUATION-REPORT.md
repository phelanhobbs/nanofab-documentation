...
## Resolution status (updated 2026-06-02; re-applied 2026-06-10)

All findings were fixed in the documentation, and a full developer-doc-vs-source validation pass was then run across **both** repos. *(2026-06-10 note: an independent re-evaluation found that fixes #1, #3–#7, and part of #2/#8 were missing from the delivered tree — they had been applied to a working copy that never landed. All eight were re-applied and re-verified on 2026-06-10.)* The executive summary above is preserved as the original as-evaluated snapshot; current status:
...
- **#8 filetransfer remote path** (found during the full source validation) — ✅ Fixed. The doc documents the deployed scripts' `/Users/phelanh/Desktop/Logs/<MACHINE>` destination, and the template's divergent Windows-style `C:\Users\...` placeholder was fixed **in source** on 2026-06-10 (UNanofabTools commit "Align FileTransferTemplate remotePath with deployed per-machine scripts"); the doc keeps a dated historical note.
...
- **`Flask-Migrate` was missing from `requirements.txt`** though `app/__init__.py` imports it at startup — ✅ Fixed: pinned `Flask-Migrate==4.1.0` (the version in the repo venv); architecture §1.2 version cell updated from "—" to `4.1.0`. Alembic remains transitive; gunicorn intentionally stays out per `09-deployment-and-operations.md`. *(2026-06-10: this fix had been left uncommitted in the working tree; it is now committed in UNanofabTools along with the pending chem-schema unique-index change.)*
- **The filetransfer template's literal `C:\…` remote path** aimed at a Debian host is a likely latent script bug — ✅ Fixed in source 2026-06-10: the template now uses the same Unix-style `/Users/phelanh/Desktop/Logs/MACHINE` form as the deployed per-machine scripts. The destination account/path model itself remains tracked as `known-issues/UNanofabTools/filetransfer.md` #1.
