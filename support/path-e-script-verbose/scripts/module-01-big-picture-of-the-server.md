# Verbose Maximal Path E - Module 01: Big Picture Of The Server
...
## Source-code pass for Module 01

READ ALOUD:

We are now doing the source-code pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Live-state pass for Module 01

READ ALOUD:

We are now doing the live-state pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Failure-mode pass for Module 01

READ ALOUD:

We are now doing the failure-mode pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Maintenance-planning pass for Module 01

READ ALOUD:

We are now doing the maintenance-planning pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Security pass for Module 01

READ ALOUD:

We are now doing the security pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Recovery pass for Module 01

READ ALOUD:

We are now doing the recovery pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Documentation-drift pass for Module 01

READ ALOUD:

We are now doing the documentation-drift pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Teach-back pass for Module 01

READ ALOUD:

We are now doing the teach-back pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Homework-review pass for Module 01

READ ALOUD:

We are now doing the homework-review pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Quarterly-audit pass for Module 01

READ ALOUD:

We are now doing the quarterly-audit pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Backup-and-restore pass for Module 01

READ ALOUD:

We are now doing the backup-and-restore pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Ownership-boundary pass for Module 01

READ ALOUD:

We are now doing the ownership-boundary pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## No-contact rehearsal pass for Module 01

READ ALOUD:

We are now doing the no-contact rehearsal pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Operator-error pass for Module 01

READ ALOUD:

We are now doing the operator-error pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Data-integrity pass for Module 01

READ ALOUD:

We are now doing the data-integrity pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
## Final-repetition pass for Module 01

READ ALOUD:

We are now doing the final-repetition pass for Big Picture Of The Server. The maintainer should connect this module to the system map, data producers, and ownership boundaries. Do not accept a vague answer. Require a named file, named command, named source path, or named live-state observation wherever possible.
...
# Read-Aloud Documentation Corpus: presentation/UNanofabTools/flaskserver/14-Security-Model.md
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/build_all.js
...
- ); const { Deck } = require(
- ); const M = require(
- ); const OUT = process.env.OUT || path.join(__dirname,
- ); fs.mkdirSync(OUT, { recursive: true }); // Optional filter: `node build_all.js 01` builds only modules whose file starts with 01 const filter = process.argv[2]; const dir = path.join(__dirname,
- ); const files = fs.readdirSync(dir).filter((f) => f.endsWith(
- )).sort(); (async () => { for (const f of files) { if (filter && !f.startsWith(filter)) continue; const mod = require(path.join(dir, f)); const d = new Deck({ title: mod.title, subtitle: M.series, author: M.presenter }); mod.build(d); const outPath = path.join(OUT, mod.filename); await d.save(outPath); console.log(


## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/deckkit.js
...
- , blur: 5, offset: 2, angle: 135, opacity: 0.12 }; } class Deck { constructor({ title, subtitle, author }) { const p = new pptxgen(); p.layout =
- ; p.author = author ||
- ; p.title = title; this.p = p; this.shortTitle = title; this.n = 0; } // crimson footer band + slide number; consistent motif on content slides _footer(slide) { slide.addShape(this.p.shapes.RECTANGLE, { x: 0, y: PH - 0.32, w: PW, h: 0.32, fill: { color: C.crimson }, line: { type:
- , { x: MX, y: PH - 0.32, w: 8, h: 0.32, margin: 0, fontFace: FONT_B, fontSize: 9, color: C.white, align:
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/00_overview_index.js

READ ALOUD OR USE AS SPEAKER NOTES:

- ); module.exports = { filename:
- Start Here — Series Overview
...
- Who should care about which parts
...
- The sixteen topics (1 of 2)
- What a server is and what ours does
- What happens when the server boots
- Where the server reads its settings
- The internal to-do list with attachments
- A page per tool, plus log downloads
- How Raspberry Pis push data in
...
- The sixteen topics (2 of 2)
- The tables and what they hold
- A standalone demo viewer page
- The Pi firmware & desktop apps
- Step-by-step path of one request
- Every address the server answers
- This overview / table of contents
...
- the on-screen content is intentionally light.
- Speaker notes are the script
- every slide has reader's notes with the full explanation.
- read the notes aloud, or read them on your own as a guide.
...
- A 30-second taste: how it all works
- The one idea everything rests on.
- a browser, a sensor, or a tool sends a request.
- it figures out what's being asked and does the work.
- a web page, some data, or a confirmation goes back.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/01_server_overview.js

READ ALOUD OR USE AS SPEAKER NOTES:

- ); module.exports = { filename:
- , build(d) { d.title_slide({ kicker:
- , presenter: M.presenter, role: M.role, dept: M.dept, date: M.date, notes:
...
- What is a server, in one sentence?
- A server is a program that runs continuously, waiting for others to ask it for things.
...
- Six jobs, all bundled into one program.
...
- Three kinds of client arrive at the same server.
- Lab staff in a browser
- log in and use HTML pages — tasks, machine data, inventory.
- small computers by machines that POST sensor readings automatically, no login.
- apps staff run that fetch data back out to visualize it.
...
- A whiteboard sketch of the whole system:
...
- nginx is the front door and handles encryption; Flask runs behind it and does the work.
...
- CSV files on disk (LogData / HSCDATA)
...
- Vocabulary, part 1: the web basics
- A message a client sends asking for something
- What the server sends back (with a status code)
- “Give me this” — page loads and data fetches
- “Here is some data” — form submissions, sensor uploads
- 200 = OK, 404 = not found, 500 = server error
- The encrypted (padlock) version of web traffic
...
- Vocabulary, part 2: the building blocks
- A specific address the server answers, e.g. /login
- A “chapter” of the server grouping related routes
- An HTML page with blanks the server fills in
- A description of one table in the database
- Plain code that does the real work behind a route
- A record that says “this person is logged in right now”
...
- If you remember one thing from this session, make it this:
...
- What this series covers — and doesn't
...
- A request and a response, concretely
- What actually crosses the wire when you open a page:
...
- GET asks for something; POST hands something over. 200 means success.
...
- Two programs, each doing what it's best at.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/02_how_it_starts.js
...
- The whole starter file, simplified:
- Read which mode to run in, build the app, and (if launched directly) start serving.
...
- create_app() assembles the server step by step.
- create the empty Flask application.
- copy the chosen configuration into the app.
- database, logins, password hashing, migrations, cross-origin access.
- plug in each feature chapter (auth, tasks, machines, API, chem...).
- serve JS/CSS files, the favicon, a date formatter.
- create any missing database tables, then return the ready app.
...
- A blueprint is a “chapter” of the server that groups related addresses.
...
- Why assemble inside a function?
- The “factory” pattern, in one idea:
...
- One recipe, several builds — no copy-pasting between modes.
...
- The same startup, two personalities — chosen by one setting.
...
- Add-ons the factory switches on
- Off-the-shelf libraries that handle common jobs.
...
- What 'register a blueprint' looks like
- Each feature chapter is plugged in with two lines:
- # repeat for: auth, admin, machines,
- Import the chapter, then plug it into the app. Seven chapters, seven plug-ins.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/04_authentication.js
...
- The path from typing your password to landing on the tasks page.
- the browser sends them over the encrypted connection.
- compare against the stored scrambled version (never plain text).
- in production, a push goes to your phone; you tap approve.
- the server records that you're logged in and sets a cookie.
- you land on the tasks page, now authenticated.
...
- We store a scrambled fingerprint, not the password itself.
...
- Two records keep you logged in.
...
- The same password always scrambles to the same fingerprint — but you can't go backwards:
- "correct horse" ──scramble──► $2b$12$Xk9... (stored)
- "correct horse" ──scramble──► $2b$12$Xk9... ✓ match
- "wrong guess" ──scramble──► $2b$12$Qp3... ✗ no match
- The server compares fingerprints, never the passwords themselves.
...
- If the database were ever stolen
...
- , steps: [ { h:
- and places it in a cookie in your browser.
- automatically, on every later request.
- so a tampered or forged cookie is rejected.
- the configured lifetime needs permanent sessions, which login does not set.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/05_admin_panel.js
...
- Authorization here is deliberately minimal.
...
- Two locks on every admin action
- Each admin route is guarded twice, in order.
- if not, you're sent to the login page first.
- if not, you're bounced back with a 'no permission' message.
...
- Each admin page is wrapped by two guards, in order:
- @login_required # 1. must be logged in
- @admin_required # 2. must be an admin
- ... show the user list ...
- Read top-down: logged-in check first, admin check second, then the action.
...
- The admin panel; managing users
- ], ], colW: [2.8, 5.6, 3.5], notes:
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/06_tasks.js
...
- , steps: [ { h:
- title, description, due date, priority, and one or more assignees.
- checks the date format and your permission to assign.
- store it, then record each assignee.
...
- How assignees work (one task, many people)
...
- Why assignees live in a separate list
- One task, several people — kept in a linking table:
- #42 "Clean trap" task #42 → Alice
- #42 ... task #42 → Bob
- #43 "Order N2" task #43 → Alice
- A person can be on many tasks; a task can have many people. The list links them.
...
- , steps: [ { h:
- only allowed kinds (docs, images, PDFs, spreadsheets) pass.
- strip anything that could let it write outside the uploads folder.
- so two files named the same don't overwrite each other.
- store the file; note its location against the task.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/07_machines_and_logs.js
...
- What the machines portal provides
...
- The portal reuses two generic builders.
...
- Downloading and graphing log files
...
- Safe downloads: the path check
- How the server stops a download from escaping its data folder.
- expand any shortcuts and '..' tricks to a true location.
- Confirm it's inside the data folder
- the real path must start with the allowed folder.
- respond 'access denied' instead of serving the file.
- If inside → serve it
...
- ALD, E-beam, MOCVD, Parylene, PECVD, Denton 635, Denton 18
...
- Each tool page is a single entry that reuses a builder:
- Seventeen tools, seventeen tiny entries — all sharing one builder.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/08_iot_api.js
...
- Addresses designed for devices to talk to, not for humans to browse.
...
- SPS30 dust sensors upload readings; apps fetch them
- Newer Pis sending particle + temp/humidity together
- The Parylene coater's Pi streams CSV batches
- Vacuum-pressure samples during a run
...
- A particle reading arrives as JSON like this (trimmed):
- : 1747876800, ' + '
- : 0.0, ... }, ' + '
- : 0.0, ... } ' + ' } ' + '}', caption:
...
- What happens to a particle reading
- confirm room, sensor, and timestamp are present.
- add a row to that sensor's CSV history file.
- overwrite that sensor's single 'most recent' row in the database.
- reply success so the Pi knows it landed.
...
- Why CSV for history, database for 'now'?
...
- The same address serves current and historical data.
- get every sensor's latest reading (for the map).
- name a room and sensor to get its full history.
- Server reads the right source
- the database for 'latest,' the CSV for history.
- a colored map, a table, or a time chart.
...
- Validation: bad data is rejected early
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/09_chemical_inventory.js
...
- The key idea: items vs. containers
- Separate the chemical from the bottle.
...
- A simplified map of the data:
- vendors ────┼─► items ──► containers ──► rooms
- (was seen during) (a scan / audit)
- Items hold many containers; each container lives in a room and appears in scans.
...
- How a walk-around inventory check is recorded.
- an audit gets its own ID and metadata (who, where, when).
- paste them or upload a file from the scanner.
- found ones are linked to the audit and stamped 'last seen now.'
- the audit records how many matched and how many didn't.
...
- Built to be safe and consistent
...
- Two honest notes for maintainers
...
- match an existing item by name, or add it.
- one unique number per bottle from the counter.
- Record each bottle + its history
- all in one all-or-nothing save.
...
- How many containers and distinct chemicals
- What expires within 30 days
- What needs an NMR check soon
- By room / vendor / system / owner
- Which active bottles were seen vs. unseen
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/10_database_models.js
...
- First, what's a 'table' and a 'model'?
...
- Stores only the most recent reading per sensor.
...
- Columns are the fields; each row is one record. Note: no real passwords, just fingerprints.
...
- Why four small databases, not one?
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/11_particle_demo.js
...
- Even a tiny file server needs a guard.
- anything containing '..' or starting with '/' is refused.
- if it exists in the demo folder.
...
- This page is mostly static — a useful contrast.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/12_consumers.js
...
- What the Pis handle on their own
...
- From a sensor reading to a chart on screen.
- and builds a small data message.
- Pi sends it to the server
- which stores it (CSV history + latest snapshot).
- Later, an app asks for data
- current readings, or one sensor's history.
- a map, a table, or a chart.
...
- What the sensor code actually sends
- A simplified version of the Pi's send step:
- room_name: "Bay C", sensor_number: "003",
- Take a reading, package it with its identity and time, send it. That's the producer.
...
- Why the room and sensor labels matter
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/13_request_lifecycle.js
...
- Every request — no matter the source — follows this path:
...
- Page, sensor upload, or inventory load — same pipeline, different handler.
...
- Example 1 — a person opens the tasks page
- browser asks for /tasks over the encrypted connection.
- not logged in → redirect to login; logged in → continue.
- your tasks, unfinished tasks, all tasks.
- the template fills in the data as HTML.
- the browser draws the tasks page.
...
- Example 2 — a sensor uploads a reading
- a Pi posts JSON to /particle-data.
- device endpoints are open on the private network.
- confirm the required fields are present.
- append to the CSV history and update the latest snapshot.
- reply success so the Pi can move on.
...
- Example 3 — the inventory page loads
- reuse a pooled connection to the chem database.
- run the query, optionally filtered by a search box.
- render the results as a sortable table.
- the inventory appears in the browser.
...
- Decorators: the checkpoints before a handler
...
- Behind the scenes: nginx and the WSGI server
...
- Status codes: the one-word verdict
- OK — here's what you asked for
- Redirect — go here instead (e.g. to the login page)
- Bad request — your data was missing or malformed
- Forbidden — you're not allowed (e.g. a blocked download)
- Not found — no such thing here
- Server error — something broke on our side
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/14_security_model.js
...
- proper resolve-then-verify path check, and the Parylene device-upload endpoints allow-list the session ID so a value like 'presentation/UNanofabTools/flaskserver/slides' can't write outside the data folder. Uploads are restricted by type and given safe, unique names. The
- Why it exists / risk
...
- If asked 'what would you fix first?', this is the order.
...
- An attacker has to beat several independent layers.
- blocked unless you can get onto the network / past nginx.
- and even then it's only a fingerprint in the database.
- you'd also need the user's physical phone.
- the signed cookie rejects tampering.
...
- Putting the gaps in perspective
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/decks/15_endpoint_reference.js

READ ALOUD OR USE AS SPEAKER NOTES:

- ); module.exports = { filename:
- , build(d) { d.title_slide({ kicker:
- Every Address, At a Glance
...
- total addresses the server answers
...
- Particle demo + static files
- ], ], colW: [7.0, 2.0, 2.9], notes:
...
- Show form / verify credentials + 2FA
- Reset via username + uNID
- ], ], colW: [3.6, 8.3], notes:
...
- Receive or return particle readings
- Receive Denton 18 pressure samples
- List and download Parylene files
...
- Relocate one or many containers
...
- Download a log file (guarded)
- Graph a log file's contents
...
- Access rules map cleanly onto the feature areas.
...
## Slide Notes From presentation/UNanofabTools/flaskserver/slides/_build/meta.js
