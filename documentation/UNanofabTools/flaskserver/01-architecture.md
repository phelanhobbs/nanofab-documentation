...
All five datastores live on the same VM. Browsers, devices, and desktop apps reach the box only through nginx on `:443`; nginx forwards to Flask on `127.0.0.1:5000`; Flask talks to the SQLite files directly and to PostgreSQL over `127.0.0.1:5432`. This was confirmed by the live-server survey ([`../liveserver/README.md`](../liveserver/README.md) §3.1 and §10); older diagrams that placed chem PostgreSQL off-box have been corrected.
...
| PostgreSQL `cheminventory` | **local on this same VM** (`postgresql@17-main`, bound to `127.0.0.1:5432`; see [`../liveserver/README.md`](../liveserver/README.md) §10) | `chem_service` | SQLAlchemy Core (`text()`) |
| `LogData/` tree | filesystem | `api`, `machines`, `data_service` (raw machine logs uploaded by the `filetransfer` scripts; sensor CSVs written by the `api` blueprint from posts originated by the [`NanofabToolkit/PicoHelperTools`](../../NanofabToolkit/PicoHelperTools/README.md) firmware) | direct file I/O |
